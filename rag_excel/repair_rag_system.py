...
def create_vector_store_enhanced() -> bool:
    """
    Carica i dati dal file Excel utilizzando l'EnhancedExcelLoader che combina
    le migliori caratteristiche di pandas, unstructured e chunking ottimizzato.
    """
    logger.info("Avvio creazione Vector Store con EnhancedExcelLoader...")
    start_time = time.time()
    try:
        # Carica i documenti dal file Excel con il loader avanzato
        logger.info(f"Caricamento documenti da: {EXCEL_FILE_PATH} con EnhancedExcelLoader")
        excel_loader = EnhancedExcelLoader(
            file_path=EXCEL_FILE_PATH,
            use_unstructured=True,  # Usa unstructured per analisi avanzata delle tabelle
            chunk_size=CHUNK_SIZE,
            chunk_overlap=CHUNK_OVERLAP
        )

        # Ottieni documenti già chunked in modo ottimale
        chunked_docs = excel_loader.get_chunked_documents()
        if not chunked_docs:
            logger.error("Nessun documento caricato dal file Excel.")
            return False
        logger.info(f"Caricati e chunked {len(chunked_docs)} documenti.")
        ...
        end_time = time.time()
        logger.info(f"Vector Store avanzato creato con successo in {end_time - start_time:.2f} secondi.")
        return True

    except Exception as e:
        logger.error(f"Errore durante la creazione del Vector Store avanzato: {str(e)}")
        import traceback
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no-enhanced", action="store_true", help="Disabilita l'uso del loader avanzato EnhancedExcelLoader.")
    parser.add_argument("--method", type=str, choices=["enhanced", "llamaparse", "optimized", "basic"], 
                        help="Specifica il metodo di creazione del vector store da utilizzare.")
    ...
    if not check_environment():
        ...
        if args.create:
            ...
            # Se è specificato un metodo specifico, usa quello
            if args.method:
                if args.method == "enhanced" and not args.no_enhanced:
                    logger.info("Utilizzo del metodo avanzato (EnhancedExcelLoader) per la creazione del Vector Store...")
                    success = create_vector_store_enhanced()
                elif args.method == "llamaparse" and LLAMA_CLOUD_API_KEY and not args.no_llamaparse:
                    logger.info("Utilizzo di LlamaParse per la creazione del Vector Store...")
                    success = create_vector_store_with_llamaparse()
                elif args.method == "optimized":
                    logger.info("Utilizzo del metodo ottimizzato per la creazione del Vector Store...")
                    success = create_vector_store_optimized()
                elif args.method == "basic":
                    logger.info("Utilizzo del metodo base per la creazione del Vector Store...")
                    success = create_vector_store()
                else:
                    logger.warning(f"Metodo {args.method} non disponibile o disabilitato. Utilizzo della sequenza di fallback.")
                    success = False

                if success:
                    logger.info(f"Creazione del Vector Store con metodo {args.method} completata con successo.")
                else:
                    logger.error(f"Creazione del Vector Store con metodo {args.method} fallita.")

            # Altrimenti, usa la sequenza di fallback
            else:
                # Prova prima con EnhancedExcelLoader se non disabilitato
                if not args.no_enhanced:
                    logger.info("Utilizzo del metodo avanzato (EnhancedExcelLoader) per la creazione del Vector Store...")
                    if create_vector_store_enhanced():
                        logger.info("Creazione del Vector Store avanzato completata con successo.")
                    else:
                        # Fallback a LlamaParse
                        logger.error("Creazione del Vector Store avanzato fallita. Tentativo con LlamaParse...")
                        if LLAMA_CLOUD_API_KEY and not args.no_llamaparse:
                            if create_vector_store_with_llamaparse():
                                ...
                                # Fallback al metodo ottimizzato
                            ...
                            # Salta LlamaParse e vai direttamente al metodo ottimizzato
                            if create_vector_store_optimized():
                                logger.info("Creazione del Vector Store ottimizzato completata con successo.")
                            else:
                                logger.error("Creazione del Vector Store ottimizzato fallita. Tentativo con metodo tradizionale...")
                                if create_vector_store():
                                    ...
                                    logger.error("Creazione del Vector Store fallita.")
                # Se EnhancedExcelLoader è disabilitato, prova con LlamaParse
                elif LLAMA_CLOUD_API_KEY and not args.no_llamaparse:
                    logger.info("Utilizzo di LlamaParse per la creazione del Vector Store...")
                    if create_vector_store_with_llamaparse():
                        logger.info("Creazione del Vector Store con LlamaParse completata con successo.")
                    else:
                        ...
                        if create_vector_store_optimized():
                            ...
                            if create_vector_store():
                                ...
                                logger.error("Creazione del Vector Store fallita.")
                # Se sia EnhancedExcelLoader che LlamaParse sono disabilitati, usa il metodo ottimizzato
                else:
                    logger.info("Utilizzo del metodo ottimizzato per la creazione del Vector Store...")
                    if create_vector_store_optimized():
                        logger.info("Creazione del Vector Store ottimizzato completata con successo.")
